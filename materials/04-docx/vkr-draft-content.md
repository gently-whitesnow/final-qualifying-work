...
3. Исходная single-node архитектура real-time контура.
4. Multi-instance архитектура без backplane.
5. Целевая архитектура с Redis backplane.
6. Последовательность доставки события `ReceiveReportPatch` между узлами.
7. Вывод проверочного клиента с diagnostics для разных серверных узлов.
...
7. Фрагмент frontend-подключения SignalR и повторного `JoinReportGroupAsync` после reconnect.
...
11. Вывод timeout для режима без Redis backplane.
