...
Структура отчёта и оформление выполнены с учётом ГОСТ 7.32-2017 [11] и ГОСТ Р 7.0.100-2018 [12], а также методических требований кафедры к подготовке отчёта по практике [13].
...
| Подход | Межузловая доставка событий | Сложность внедрения | Ограничения |
| --- | --- | --- | --- |
| Один экземпляр backend-сервиса | Нет | Низкая | Нет горизонтального масштабирования, один узел является точкой отказа |
| Несколько экземпляров без backplane | Нет | Низкая или средняя | Real-time пространство фрагментируется по узлам |
| Sticky sessions | Нет, если нет отдельной синхронизации | Средняя | Удерживает клиента на узле, но не доставляет события другим узлам |
| Redis backplane для SignalR | Да | Средняя | Подходит для transient UI-событий, зависит от Redis и сетевой задержки |
| Broker/event bus | Да | Высокая | Требует проектирования схем событий, persistence и consumer-групп |
| Managed real-time service | Да | Средняя | Уменьшает контроль над self-hosted серверным контуром |

Для выбранной цели принят подход SignalR + Redis backplane. Он соответствует self-hosted характеру стенда, не требует радикальной перестройки доменной логики и позволяет экспериментально проверить межузловую доставку событий [4], [9].
...
Работы по реализации были разделены на три направления: изменение backend-конфигурации SignalR, подготовка инфраструктуры multi-instance стенда и разработка проверочного клиента для фиксации результатов испытаний.
...
| Серия из 5 итераций | Да | Все события доставлены | 5 из 5, p95 = 11,6 мс | Подтверждена повторяемость |
| Rejoin после разрыва | Да | Клиент повторно подписывается | Событие после rejoin получено, 6,6 мс | Подтверждена повторная подписка |
