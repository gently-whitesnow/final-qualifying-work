...
def deck_slides(kind: str = "vkr") -> list[str]:
    ...
    return [
        ...
        # 5. Что сделано
        title(10, "Что сделано", "Подключён межузловой канал, добавлена диагностика, собран стенд и клиент проверки")
        + shape(20, 0.75, 1.7, 5.85, 2.4,
                "1 · Межузловая доставка событий\n\n"
                "К SignalR подключён Redis backplane. "
                "Включается переменной окружения — один и тот же код "
                "работает и в режиме с межузловым каналом, и без него.",
                fill=PANEL, size=14)
        + shape(21, 0.75, 4.20, 5.85, 2.05,
                "2 · Диагностика подключения\n\n"
                "Узел сообщает свой идентификатор и идентификатор соединения; "
                "ключевые события жизненного цикла соединения логируются.",
                fill=PANEL, size=14)
        + shape(22, 6.75, 1.7, 5.85, 2.4,
                "3 · Многоэкземплярный стенд\n\n"
                "Два экземпляра backend за nginx, общая база данных и Redis, "
                "отдельный режим без межузлового канала — для воспроизведения исходной проблемы.",
                fill=PANEL, size=14)
        + shape(23, 6.75, 4.20, 5.85, 2.05,
                "4 · Проверочный клиент\n\n"
                "Подключается к разным узлам, фиксирует доставку события "
                "и поддерживает четыре сценария проверки.",
                fill=PANEL, size=14),
