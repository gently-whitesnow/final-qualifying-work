...
def footer(n: int, total: int) -> str:
    tagline = shape(900, 0.65, 7.12, 8.0, 0.22, "В реальном времени", fill="", line="", size=8, text_color=MUTED)
    number = shape(901, 4.65, 7.02, 8.0, 0.32, f"{n}/{total}", fill="", line="", size=12, bold=True, text_color=MUTED, align="r")
    return tagline + "\n" + number


def slide_xml(n: int, body: str, total: int) -> str:
        ...
        slides.extend(
            [
                ...
                + shape(21, 0.75, 3.30, 11.85, 1.35,
                        "AI меняет ожидания от интерфейсов\n\n"
                        "Взаимодействие всё чаще идёт через диалог с ассистентом: он сам создаёт комментарий, "
                        "меняет статус, предлагает исполнителя — и пользователь ждёт мгновенного отклика.",
                        fill=IRIS_SOFT, line=ACCENT_2, size=14)
                + shape(22, 0.75, 4.85, 11.85, 1.45,
                        "Real-time — это инженерная характеристика, а не «фича»\n\n"
                        "Отзывчивость интерфейса требует масштабируемого и отказоустойчивого контура — "
                        "на нескольких экземплярах серверного сервиса, а не на одном.",
...
def build(kind: str = "vkr") -> Path:
    ...
    with zipfile.ZipFile(out_path, "w", compression=zipfile.ZIP_DEFLATED) as z:
        ...
        for i, slide in enumerate(slides, start=1):
            ...
            z.writestr(f"ppt/slides/slide{i}.xml", slide_xml(i, body, len(slides)))
