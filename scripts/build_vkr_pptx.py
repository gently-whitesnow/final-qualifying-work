...
def sdlc_slide() -> str:
    return (
        ...
        + shape(32, 0.75, 5.55, 11.85, 1.0,
                "Они работают с одним отчётом одновременно — значит, изменения должны быть видны сразу, без F5.",
                fill=SOFT, line=ACCENT, size=15, bold=True, align="ctr")
    )
        ...
        slides.extend(
            [
                ...
                + shape(20, 0.75, 1.65, 3.85, 4.75,
                        ...
                        "Современные веб-системы объединяют связанные сущности, "
                        "фоновые операции, уведомления и совместное редактирование.\n\n"
                        "Изменения должны быть видны участникам без ручного обновления страницы.",
                        fill=PANEL, size=13)
                + shape(21, 4.80, 1.65, 3.85, 4.75,
                        "AI-инструменты как отраслевой контекст\n\n"
                        "Интеллектуальные ассистенты могут сами создавать комментарии, "
                        "обновлять статус, предлагать исполнителя, дополнять описание задачи.\n\n"
                        "Ожидание живой синхронизации усиливается даже у ручных сценариев.",
                        fill=IRIS_SOFT, line=ACCENT_2, size=13)
                + shape(22, 8.85, 1.65, 3.85, 4.75,
                        "Что это значит для инженера\n\n"
                        "Real-time перестаёт быть «фичей» и становится инженерной характеристикой: "
                        "способность интерфейса быстро и согласованно распространять изменения.\n\n"
                        "Поэтому строить его нужно для нескольких экземпляров backend, а не только single-node.",
                        fill=SOFT, line=ACCENT, size=13, bold=True)
                + shape(23, 0.75, 6.55, 11.85, 0.4,
                        "В работе AI-функциональность не утверждается как реализованная; это контекст актуальности.",
                        fill="", line="", size=11, text_color=MUTED, align="ctr"),
            ]
        )

    slides.extend([
        # 5. Проблема — через сценку
        title(10, "В чём проблема",
              "Когда backend один — всё работает. Когда их несколько — изменения теряются.")
        + shape(20, 0.75, 1.55, 11.85, 1.05,
                "Тестировщик и разработчик открыли один и тот же отчёт. "
                "Тестировщик меняет статус — у разработчика в браузере ничего не происходит.",
                fill=SOFT, line=ACCENT, size=17, bold=True, align="ctr")
        + shape(21, 0.75, 2.85, 5.85, 3.6,
                "Почему так\n\n"
                "Каждый экземпляр backend помнит только своих клиентов. "
                "Если тестировщик и разработчик подключены к разным экземплярам, "
                "событие об изменении до второго просто не доходит.",
                fill=PANEL, size=14)
        + shape(22, 6.75, 2.85, 5.85, 3.6,
                "Что это значит на практике\n\n"
                "Совместная работа над отчётом ломается: часть участников видит новый статус, "
                "часть — старый.\n\n"
                "Backend нельзя масштабировать горизонтально, не перестроив доставку событий.",
                fill=PANEL, size=14)
        + shape(23, 0.75, 6.60, 11.85, 0.35,
                "Технически: подключения и группы SignalR хранятся в памяти процесса.",
                fill="", line="", size=11, text_color=MUTED, align="ctr"),

        # 6. Цель и задачи
        title(10, f"{goal_label}",
              "Сделать так, чтобы изменения доходили до всех — независимо от того, к какому экземпляру подключён пользователь")
        + shape(20, 0.75, 1.75, 11.85, 1.35,
                "Real-time интерфейс, в котором при нескольких экземплярах backend "
                "изменения в отчёте доставляются всем участникам, открывшим этот отчёт.",
                fill=SOFT, line=ACCENT, size=16, bold=True, align="ctr")
        + shape(21, 0.75, 3.45, 3.85, 2.85,
                "1. Спроектировать\n\n"
                "Архитектуру распределённой доставки событий и её границы.",
                fill=PANEL, size=14)
        + shape(22, 4.80, 3.45, 3.85, 2.85,
                "2. Реализовать\n\n"
                "Backend, фронтенд и стенд из двух экземпляров за общей точкой входа.",
                fill=PANEL, size=14)
        + shape(23, 8.85, 3.45, 3.85, 2.85,
                "3. Проверить\n\n"
                "Воспроизвести проблему, подтвердить решение, измерить задержку и устойчивость.",
                fill=PANEL, size=14),
        ...
        + shape(142, 8.55, 5.22, 4.30, 0.70, "Требует переработки доменных событий",
                fill=PANEL, size=12, text_color=MUTED, margin=10)
        + shape(150, 0.55, 6.00, 6.40, 0.70, "6.  Управляемый real-time сервис",
                fill=PANEL, size=12, margin=10)
        ...
        + shape(25, *rds, "Redis backplane", fill=IRIS_SOFT, line=ACCENT_2, size=14, bold=True, align="ctr")
        ...
        title(10, "Что происходит при изменении отчёта",
              "Шесть шагов от действия пользователя до клиента на другом экземпляре (§3.3, рис. 6 ВКР)")
        ...
        + shape(21, 2.65, 1.95, 1.95, 4.20,
                "2. HTTP-запрос\n\n"
                "Через nginx запрос попадает на один из экземпляров — app-api-1.",
                fill=PANEL, size=12)
        ...
        + shape(20, 0.55, 1.70, 2.40, 4.80,
                "1. Серверная конфигурация\n\n"
                "Подключение Redis backplane через переменную окружения. "
                "Один и тот же код запускается с межузловым каналом и без него.",
                fill=PANEL, size=12)
        + shape(21, 3.00, 1.70, 2.40, 4.80,
                "2. Хаб и отправка событий\n\n"
                "Диагностический метод, идентификатор экземпляра, eventId, "
                "централизованная отправка через единый клиент хаба.",
                fill=PANEL, size=12)
        + shape(22, 5.45, 1.70, 2.40, 4.80,
                "3. Клиентский real-time слой\n\n"
                "WebSocket-транспорт, автоматическое переподключение и "
                "повторное вступление в группу текущего отчёта.",
                fill=PANEL, size=12)
        + shape(23, 7.90, 1.70, 2.40, 4.80,
                "4. Инфраструктура стенда\n\n"
                "Docker Compose, два экземпляра app-api, общий nginx, "
                "Redis и PostgreSQL. Отдельный override для режима без backplane.",
                fill=SOFT, line=ACCENT, size=12, bold=True)
        + shape(24, 10.35, 1.70, 2.40, 4.80,
                "5. Проверочный клиент\n\n"
                "Node.js-скрипт, который подключается к двум узлам, "
                "ждёт событие и измеряет задержку доставки.",
                fill=IRIS_SOFT, line=ACCENT_2, size=12, bold=True)
        + shape(25, 0.55, 6.70, 12.30, 0.35,
                ".NET / ASP.NET Core · SignalR · StackExchange.Redis · React · TypeScript · Node.js · nginx · Docker Compose",
                fill="", line="", size=10, text_color=MUTED, align="ctr"),

        # Стенд испытаний — §4.5 + §5.2 ВКР
        title(10, "Стенд испытаний",
              "Минимальный воспроизводимый Docker Compose-стенд (§4.5, §5.2 ВКР)")
        + shape(20, 0.75, 1.65, 5.85, 4.85,
                "Состав стенда\n\n"
                "•  postgres_app_thesis — общая база\n"
                "•  redis_app_thesis — общий канал событий\n"
                "•  app-api-1 — серверный экземпляр №1\n"
                "•  app-api-2 — серверный экземпляр №2\n"
                "•  nginx_app_thesis — общая точка входа\n"
                "•  Node.js-скрипт проверки — вне продукта\n\n"
                "Оба экземпляра app-api собраны из одного Dockerfile и отличаются "
                "только значением SERVER_INSTANCE_ID.",
                fill=PANEL, size=13)
        + shape(21, 6.75, 1.65, 5.85, 4.85,
                "Как запускают\n\n"
                "С общим каналом (целевой режим):\n"
                "docker compose -f docker-compose.thesis.yml up\n\n"
                "Без общего канала (контрольный режим):\n"
                "docker compose -f thesis.yml -f thesis.no-backplane.yml up\n\n"
                "Проверочный клиент:\n"
                "node scripts/realtime-scaleout-check.mjs\n\n"
                "Серия и сценарии:\n"
                "THESIS_ITERATIONS=30  ·  THESIS_SCENARIO=rejoin  ·  failover",
                fill=SOFT, line=ACCENT, size=12, bold=False)
        + shape(22, 0.75, 6.65, 11.85, 0.35,
                "Эксперимент проверяет именно серверный real-time контур и межузловую доставку — "
                "независимо от полноценного UI продукта.",
                fill="", line="", size=11, text_color=MUTED, align="ctr"),

        # Программа испытаний — §5.1 ВКР
        title(10, "Программа испытаний",
              "Один код в двух режимах, пять контрольных проверок (§5.1 ВКР)")
        + shape(20, 0.75, 1.65, 5.85, 4.95,
                "Два режима\n\n"
                "•  Без Redis backplane — контрольный режим, воспроизводит исходное ограничение.\n\n"
                "•  С Redis backplane — целевой режим, проверяет решение.\n\n"
                "Код приложения и сценарий идентичны. Различие — только в значении "
                "REDIS_CONNECTION_STRING; это даёт причинную связь, а не корреляцию.",
                fill=PANEL, size=14)
        + shape(21, 6.75, 1.65, 5.85, 4.95,
                "Пять проверок\n\n"
                "1.  Одиночная доставка без backplane — ожидание тайм-аута.\n\n"
                "2.  Одиночная доставка с backplane — подтверждение решения.\n\n"
                "3.  Серия из 30 итераций — повторяемость и задержка.\n\n"
                "4.  Повторная подписка после разрыва соединения.\n\n"
                "5.  Переключение на другой экземпляр после отказа узла.",
        ...
        title(10, "Расширенные сценарии испытаний",
              "Серийная проверка задержки и устойчивость к разрывам/отказам (§5.5–5.6 ВКР)")
        + shape(20, 0.75, 1.75, 4.0, 4.65,
                "§5.5 Серийная проверка\n\n"
                "30 итераций, режим с backplane.\n\n"
                "Успешно: 30 из 30\n"
                "avg 7,2 мс  ·  p50 6,1 мс\n"
                "min 3,8 мс  ·  p95 13,8 мс\n\n"
                "Повторяемость доставки и порядок задержки в локальном стенде.",
                fill=PANEL, size=13)
        + shape(21, 4.85, 1.75, 4.0, 4.65,
                "§5.6 Повторная подписка\n\n"
                "Сценарий rejoin: разрыв соединения, новое подключение, повторное вступление в группу отчёта.\n\n"
                "переподключение 6,5 мс\n"
                "доставка после rejoin 15,2 мс\n\n"
                "Контур устойчив к разрывам клиентского соединения.",
                fill=PANEL, size=13)
        + shape(22, 8.95, 1.75, 4.0, 4.65,
                "§5.6 Переключение после отказа\n\n"
                "Сценарий failover: остановка одного экземпляра, переключение через nginx на оставшийся.\n\n"
                "переключение 352,0 мс\n"
                "доставка после failover 5,4 мс\n\n"
        ...
        + shape(80, 0.55, 1.55, 4.85, 0.40, "Сценарий",
                fill="", line="", size=11, bold=True, text_color=MUTED, margin=8)
        + shape(81, 5.50, 1.55, 1.10, 0.40, "Backplane",
                fill="", line="", size=11, bold=True, text_color=MUTED, align="ctr", margin=4)
        + shape(82, 6.70, 1.55, 3.75, 0.40, "Фактический результат",
                fill="", line="", size=11, bold=True, text_color=MUTED, margin=8)
        + shape(83, 10.55, 1.55, 2.30, 0.40, "Вывод",
                fill="", line="", size=11, bold=True, text_color=MUTED, margin=8)
        + shape(200, 0.55, 2.00, 4.85, 0.75, "Доставка без Redis backplane",
                fill=PANEL, size=12, margin=10)
        ...
        + shape(210, 0.55, 2.80, 4.85, 0.75, "Доставка с Redis backplane",
                fill=PANEL, size=12, margin=10)
        + shape(211, 5.50, 2.80, 1.10, 0.75, "Да",
                fill=SOFT, line=ACCENT, text_color=ACCENT, size=12, bold=True, align="ctr", margin=6)
        + shape(212, 6.70, 2.80, 3.75, 0.75, "ok: true · задержка ≈ 9,5 мс",
                fill=PANEL, size=12, margin=10)
        ...
        + shape(221, 5.50, 3.60, 1.10, 0.75, "Да",
                fill=PANEL, text_color=ACCENT, size=12, bold=True, align="ctr", margin=6)
        + shape(222, 6.70, 3.60, 3.75, 0.75, "30 из 30 · p95 = 13,8 мс",
                fill=PANEL, size=12, margin=10)
        ...
        + shape(231, 5.50, 4.40, 1.10, 0.75, "Да",
                fill=PANEL, text_color=ACCENT, size=12, bold=True, align="ctr", margin=6)
        + shape(232, 6.70, 4.40, 3.75, 0.75, "Событие после rejoin · 15,2 мс",
                fill=PANEL, size=12, margin=10)
        ...
        + shape(241, 5.50, 5.20, 1.10, 0.75, "Да",
                fill=PANEL, text_color=ACCENT, size=12, bold=True, align="ctr", margin=6)
        + shape(242, 6.70, 5.20, 3.75, 0.75, "app-api-2 → app-api-1, событие получено",
                fill=PANEL, size=12, margin=10)
        ...
        title(10, "Чего решение не делает",
              "Это интерфейсная синхронизация, а не строгий промышленный real-time")
        + shape(20, 0.75, 1.65, 3.85, 4.75,
                "Задержка не нулевая\n\n"
