...
## Проверка целостности ВКР

После каждой пересборки сверять, что фактический PDF соответствует тому, что обещают РЕФЕРАТ и СОДЕРЖАНИЕ. Хардкоженные значения в [`scripts/build_vkr_docx.py`](scripts/build_vkr_docx.py) (объем, число рисунков/таблиц/источников и страницы приложений) не пересчитываются автоматически.

```bash
cd /Users/gently/projects/final-qualifying-work
.venv/bin/python3.12 scripts/verify_vkr.py
```

Скрипт сравнивает:

- число страниц PDF с фразой «содержит N страниц» в реферате;
- число рисунков, таблиц, источников, приложений с теми же числами в реферате;
- страницы пунктов содержания (ЗАКЛЮЧЕНИЕ, СПИСОК, ПРИЛОЖЕНИЕ А/Б/В) с фактическим расположением заголовков в PDF (с учетом смещения титульный лист + задание).

Если что-то расходится — поправить константы в `add_abstract` и `add_toc` в `scripts/build_vkr_docx.py`, пересобрать DOCX и снова прогнать рендер и проверку.
